chrome.extension.sendMessage({}, function (response) {
	var readyStateCheckInterval = setInterval(function () {
		if (document.readyState === "complete") {
			...
			console.log('Surfety: sentinel is up and running');

			// ----------------------------------------------------------

			chrome.storage.sync.get(['surfety_user_state'], async function (storage) {
				if (storage.surfety_user_state) {
					console.log('user state: ' + storage.surfety_user_state);
					if (storage.surfety_user_state == "logged_in") {
						await sentinel();
						//console.log("tu sei dopo sentinel");
...
var user_interaction_weights = {};
var navigated_page = null;

//function to get keywords if user navigate on privacy page
function getKeywords(payload) {
	var idx = lunr(function () {
		this.field('body');
		this.add(
			{
				"body": payload.page_content,
			});
	});
	payload.privacy_policy.url = payload.url;
	payload.privacy_policy.content = payload.page_content;
	for (var i = 0; i < comparators.keyword.length; i++) {
		if ((new RegExp(comparators.keyword[i].value, "i").test(payload.page_content))) {
			payload.privacy_policy.keywords.push(comparators.keyword[i].value);
			payload.privacy_policy.score += parseInt(comparators.keyword[i].score, 10);
			//console.log(comparators.keyword[i].value);
		} else {
			var matches = idx.search(comparators.keyword[i].value);
			var threshold = 85;
			if (matches.length > 0) {
				matches.forEach(function (match) {
					if ((match.score * 100 > threshold)) {
						payload.privacy_policy.keywords.push(comparators.keyword[i].value);
						payload.privacy_policy.score += parseInt(comparators.keyword[i].score, 10);
					}
...
//function to get privacy content and its keywords if user does not navigate on privacy page
function getPrivacyContentAndKeywords(payload) {
	chrome.extension.sendRequest(payload.privacy_policy.url,
		function (response) {
			payload.privacy_policy.content = response;
			var idx = lunr(function () {
				this.field('body');
				this.add(
					{
			...
			for (var i = 0; i < comparators.keyword.length; i++) {
				if ((new RegExp(comparators.keyword[i].value, "i").test(payload.privacy_policy.content))) {
					payload.privacy_policy.keywords.push(comparators.keyword[i].value);
					payload.privacy_policy.score += parseInt(comparators.keyword[i].score, 10);
					//console.log(comparators.keyword[i].value);
				} else {
...
function sentinel() {
	...
	navigated_page = document.body.innerText;
   ...
   */
	...
	if (payload.url.indexOf('/privacy-policy') > -1 || payload.url.indexOf('/privacy') > -1 || payload.url.indexOf('privacy.html') > -1 || payload.url.indexOf('privacy-policy.html') > -1 || payload.url.indexOf('privacy.html') > -1) {
		...
		return getKeywords(payload);
	}
	else {
		// check if user lands on a page that has a reference to privacy-policy page
		var a_nodes = document.querySelectorAll('a[href*=privacy]'); // -> query all the DOMs in order to retrieve the latest "a" element whose "href" attribute value contains the substring "privacy" (usually at the bottom/footer of the page)
		if (a_nodes.length > 0) {
			payload.privacy_policy.url = a_nodes[a_nodes.length - 1].href;
			payload.privacy_policy.url = payload.privacy_policy.url.replace(/^http:\/\//i, 'https://');
			//console.log(getPrivacyContentAndKeywords(payload))
			return getPrivacyContentAndKeywords(payload);
		}
	}
}

function watcher() {
	present_page = JSON.stringify(document.body.innerText);
	past_page = JSON.stringify(navigated_page);
	threshold = 90;
	console.log('lev_dist ', JaroWrinker(present_page, past_page) * 100);
	if (JaroWrinker(present_page, past_page) * 100 < threshold) {
		sentinel();
	}
}
	...
	return new Promise(function (resolve, reject) {
		chrome.storage.sync.get(['surfety_oauth_access_token', 'surfety_oauth_exp_date'], async function (storage) {
			if (storage.surfety_oauth_access_token && storage.surfety_oauth_exp_date) {
				...
				// check expiration token, if false refresh
				if (now < exp_date) {
					var XHR = new XMLHttpRequest();
					XHR.open('POST', 'https://api.surfety.it/api/navigation', true);
					XHR.setRequestHeader('Authorization', 'Bearer ' + storage.surfety_oauth_access_token);
					...
					XHR.onload = function () {
						if (XHR.readyState == 4 && XHR.status == '200') {
							...
							console.error(XHR.responseText); //response undefined
					...
					if (result) { navigation(payload); }
					else {
						reject("Navigation error");
					}
				}
			} else {
				reject("No navigation tokens found");
			}
		})
	}).catch((err) => {
		console.log(err);
		...
		chrome.storage.sync.get(['surfety_oauth_refresh_token'], function (storage) {
			if (storage.surfety_oauth_refresh_token) {
					...
					if (XHR.readyState == 4 && XHR.status == '200') {
						...
						if (response.original && response.original == "Oauth token is null or invalid.") {
							...
							chrome.storage.sync.set({ 'surfety_user_state': 'logged_out' }, function () {
								...
								console.log('logged_out in sentinel@refresh1');
						...
						chrome.storage.sync.set({ 'surfety_user_state': 'logged_out' }, function () {
							console.log('oauth token is null or invalid');
							console.log('logged_out in sentinel@refresh2');
							reject('oauth token is null or invalid');
						});
					}
				};
				...
				chrome.storage.sync.set({ 'surfety_user_state': 'logged_out' }, function () {
					...
					console.log('logged_out in sentinel@refresh3');
...
fetch(chrome.extension.getURL('/json/privacy_policy.json')).then((resp) => resp.json()).then(function (jsonData) {
	comparators.privacy_policy = jsonData;
});
